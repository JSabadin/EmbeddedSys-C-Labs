...
int main(int argc, char **argv) // ODJEMALEC - PC
{
  ...
  if ((fd = open("video.h264", O_RDWR)) < 0)
  {
    printf("napaka open\n");
    exit(1);
  }
  ...
  if ((sd = initClient("192.168.91.55")) < 0)
  {
    printf("napaka init\n");
    exit(1);
  }
...
// 2. gst-launch-1.0 filesrc location=video ! h264parse ! avdec_h264 ! videoconvert ! autovideosink
...
//  2.
//   gst-launch-1.0 -v filesrc location=video.h264 ! h264parse ! avdec_h264 ! autovideosink sync=false